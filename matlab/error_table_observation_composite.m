...
for particle_count_c = particle_counts
    ...
    T = struct2table(M);
    writetable(T, fullfile(table_path, ['mse_table_observation_composite_' particle_count_str '.xlsx']));
    ...
    tex_name = fullfile(table_path, ['mse_table_observation_composite_' particle_count_str '.tex']);
    file_id = fopen(tex_name, 'w');
    [nrows,ncols] = size(latex);
    for row = 1:nrows
        new_text = strrep(latex{row,:}, '_', ' ');
        new_text = strrep(new_text, 'MyTableCaption', ...
        ['Composite MSE Over Observation Methods for \(N=' num2str(particle_count) '\)']);
        new_text = strrep(new_text, 'MyTableLabel', ['mse_observation_composite' num2str(particle_count)]);
        fprintf(file_id, '%s\n', new_text);
    end
    fclose(file_id);
end