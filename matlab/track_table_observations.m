...
for particle_count_c = particle_counts
    ...
    T = struct2table(M);
    writetable(T, fullfile(table_path, ['track_table_observations' particle_count_str '.xlsx']));
    ...
    tex_name = fullfile(table_path, ['track_table_observations_' particle_count_str '.tex']);
    file_id = fopen(tex_name, 'w');
    [nrows,ncols] = size(latex);
    for row = 1:nrows
        new_text = strrep(latex{row,:}, '_', ' ');
        new_text = strrep(new_text, 'MyTableCaption', ...
        ['Average track length across observation methods for \(N=' num2str(particle_count) '\)']);
        new_text = strrep(new_text, 'MyTableLabel', ['average_track_observation' num2str(particle_count)]);
        fprintf(file_id, '%s\n', new_text);
    end
    fclose(file_id);
end